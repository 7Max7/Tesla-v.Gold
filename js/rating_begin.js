...
var objImagePreloader=new Image();objImagePreloader.onload=function(){var newWidth=objImagePreloader.width;var newHeight=objImagePreloader.height;if(settings.fitToScreen){var arrayPageSize=___getPageSize();var ratio;var initialPageWidth=arrayPageSize[2]-2*settings.containerBorderSize;var initialPageHeight=arrayPageSize[3]-85;var dI=initialPageWidth/initialPageHeight;var dP=objImagePreloader.width/objImagePreloader.height;if((objImagePreloader.height>initialPageHeight)||(objImagePreloader.width>initialPageWidth)){if(dI>dP){newWidth=parseInt((initialPageHeight/objImagePreloader.height)*objImagePreloader.width);newHeight=initialPageHeight;}else{newHeight=parseInt((initialPageWidth/objImagePreloader.width)*objImagePreloader.height);newWidth=initialPageWidth;}}}
$('#lightbox-image').attr('src',settings.imageArray[settings.activeImage][0]).width(newWidth).height(newHeight)
...
// End
$j = jQuery.noConflict(); // Nu stricam moshneagu lib.js %)
...
function initSpoilers()
{
	$j('div.sp-head').not('.jsClickEvAttached').click(function(e){
			var $sp_body = $j('div.sp-body:first', $j(this).parent());
