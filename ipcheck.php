...
if (get_user_class() >= UC_MODERATOR)
{
 $res = sql_query("SELECT count(*) AS dupl, ip FROM users WHERE enabled = 'yes' AND ip <> '' AND ip <> '127.0.0.0' GROUP BY ip ORDER BY dupl DESC, ip") or sqlerr(__FILE__, __LINE__);
  print("
 <tr align=center>
 <td class=colhead width=10>C/P</td>
 <td class=colhead width=90>Пользователь</td>
 <td class=colhead width=70>Почта</td>
 <td class=colhead width=70>Регистрация</td>
 <td class=colhead width=75>Активность</td>
 <td class=colhead width=70><img title=\"Скачал\" src=\"pic/browse/down.gif\" border=\"0\"></td>
 <td class=colhead width=70><img title=\"Раздал\" src=\"pic/browse/up.gif\" border=\"0\"></td>
 <td class=colhead width=45>Рейтинг</td>
 <td class=colhead width=125>IP</td>
 <td class=colhead width=40>Пир</td>
 </tr>\n");
 $uc = 0;
  while($ras = mysql_fetch_assoc($res)) {
        if ($ras["dupl"] <= 1)
          break;
        if ($ip <> $ras['ip']) {
          $ros = sql_query("SELECT id, status, username, class,  email, added, last_access, downloaded, uploaded, ip, warned, donor, enabled, (SELECT COUNT(*) FROM peers WHERE peers.ip = users.ip AND users.id = peers.userid) AS peer_count FROM users WHERE ip='".$ras['ip']."' ORDER BY id") or sqlerr(__FILE__, __LINE__);
          ...
          if ($num2 > 1) {
            ...
            while($arr = mysql_fetch_assoc($ros)) {
            	 ...
            	 {$status="<img style=\"border:none\" alt=\"Подтвержден\" title=\"Подтвержден\" src=\"pic/ok.gif\">";}
            	 else
            	  {$status="<img style=\"border:none\" alt=\"Не подтвержден\" title=\"Не подтвержден\" src=\"pic/error.gif\">";}
    ...
    if ((get_user_class() <= UC_ADMINISTRATOR) && $arr["class"] == UC_SYSOP)
	{  
	$arr[email] = 'Скрыта';
	}
	else
    ...
    $arr[email] = $arr[email];
                ...
                <td align=left><b><a href='userdetails.php?id=" . $arr['id'] . "'>" . get_user_class_color($arr['class'], $arr['username'])."</b></a>" . get_user_icons($arr) . "</td>
