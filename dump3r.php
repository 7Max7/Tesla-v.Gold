...
if (!empty($_POST['login']) && isset($_POST['pass'])) {
	if (@mysql_connect(DBHOST, $_POST['login'], $_POST['pass'])){
		setcookie("sxd", base64_encode(base64_encode("SKD101:{$_POST['login']}:{$_POST['pass']}")));
		header("Location: dump3r.php");
		mysql_close();
		exit;
	}
	else{
...
elseif (!empty($_COOKIE['sxd'])) {
    $user = explode(":", base64_decode(base64_decode($_COOKIE['sxd'])));
...
class dumper {
	...
	function backup() {
		...
		if ($this->mysql_version > 40101 && CHARSET != 'auto') {
			mysql_query("SET NAMES 'cp1251'",$lastlink) or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysql_error(), E_USER_ERROR);
	...
	function main(){
		...
		$buttons = "<INPUT TYPE=submit VALUE=Применить><INPUT TYPE=button VALUE=Выход onClick=\"location.href = 'dump3r.php?reload'\">";
...
<B><A HREF="index.php" STYLE="color: white; text-decoration: none;">Вернутся на трекер</A></B><IMG ID=GS WIDTH=1 HEIGHT=1 STYLE="visibility: hidden;"></TD>
</TR>
<TR>
<FORM NAME=skb METHOD=POST ACTION=dump3r.php>
