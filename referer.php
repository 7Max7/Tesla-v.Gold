...
//sql_query("UPDATE referrers AS t SET t.uid = (SELECT id FROM users AS ts WHERE ts.ip=t.ip LIMIT 1) WHERE t.uid='0'")or sqlerr(__FILE__,__LINE__);
...
if ($d=="one"){
stdheadchat("Удаление реферала с базы id: $id");
...
stdheadchat("Удаление рефералов с базы id: $id");
...
stdheadchat("Просмотр реферала с ".$sqass["parse_url"]);
}
else
{
...
///ALTER TABLE `referrers` ADD INDEX `count` ( `parse_url` ) 


stdheadchat("ТОП сайтов, от которых мы получаем посетителей");
