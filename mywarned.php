...
if ($_SERVER["REQUEST_METHOD"] == "POST")
 {
    ...
    if ($CURUSER["uploaded"] < ($id*$STOIMOST))
     {
        stdmsg($tracker_lang['error'], "У вас недостаточно аплоада!");
        die();
    }

    $modcomment = sqlesc(gmdate("Y-m-d") . " - Обменял " .$id. " предупреждений на ".mksize($id*$STOIMOST)."\n " . $CURUSER['usercomment']);
...
<?
    ...
    {    $id = $i;
        ...
        $img.="<img src=\"".$pic_base_url."warned.gif\" alt=\"Уровень предупреждений\" title=\"Уровень предупреждений\">";
        ...
        $descr ="Обменять ".$i." предупреждени$i2";
...
?>
    <tr align="center"><td class="colhead" colspan="3">Мои предупреждения - <?=$CURUSER["num_warned"];?> штук(и), аплоада в наличии <?=mksize($CURUSER["uploaded"]);?></td></tr>
